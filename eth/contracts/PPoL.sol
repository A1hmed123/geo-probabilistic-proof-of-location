...
contract PPoL is Ownable, RingMixerV2 {
  ...
  function verifyingNodesCount() public view returns(uint256) {
    return verifyingNodes.length;
  }

  function ringSign(bytes _message, address _fromVNode, uint256 _prvKey, address[] _dummyNodes, uint256[] _randNums)
    public view returns(uint256[32] signature, bool success)
  {
    uint256[32] memory empty;
    if (!isVerifyingNode(_fromVNode)
        || _dummyNodes.length != _randNums.length
        || _dummyNodes.length.mul(2).add(1) >= 32)
    {
      return (empty, false);
    }

    uint256 N = _dummyNodes.length;
    ...
    for (i = 0; i < N; i = i.add(1)) {
      if (!isVerifyingNode(_dummyNodes[i])) { return (empty, false); }
      data[i.add(2)] = _randNums[i];
      data[i.add(3).add(N)] = nodePublicKey[_dummyNodes[i]];
    }
    return (RingSign(_message, data), true);
  }

  function verifyProof(uint256 _blockNum, bytes32 _blockHash, string _uid, address _userAddr, bytes _userSig,
    uint256[] _ringSig)
    public view returns(bool isValid)
  {
    return ECRecovery.recover(keccak256(_blockNum, _blockHash, _uid), _userSig) == _userAddr
      && RingVerify(_userSig, _ringSig);
  }

  function logProof(uint256 _blockNum, bytes32 _blockHash, string _uid, address _userAddr, bytes _userSig,
    uint256[] _ringSig)
